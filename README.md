...
![Tests](https://img.shields.io/badge/tests-194%20unit%20%2B%2024%20e2e-brightgreen)
...
to sign up for and no API key to obtain: clone, install, seed, run.
...
Deliberately *not* used: no Redux (server state is a cache, not app state), no component
...
**194 unit tests** cover the code where a wrong answer is silent: regression and
...
Point `DATABASE_URL` at a [Turso](https://turso.tech) database, set `DATABASE_AUTH_TOKEN`
